...
create table foto_produto (id bigserial not null, content_type varchar(255), descricao varchar(255), nome_arquivo varchar(255), tamanho float4 not null, produto_id bigint not null, primary key (id));
...
create table item_pedido (observacao varchar(255), preco_total numeric(38,2), preco_unitario numeric(38,2), quantidade integer not null, pedido_id bigint not null, produto_id bigint not null, primary key (pedido_id, produto_id));
create table pedido (id bigserial not null, data_cancelamento timestamp(6), data_confirmacao timestamp(6), data_criacao timestamp(6), data_entrega timestamp(6), endereco_bairro varchar(255), endereco_cep varchar(255), endereco_complemento varchar(255), endereco_logradouro varchar(255), endereco_numero varchar(255), status smallint, sub_total numeric(38,2), taxa_frete numeric(38,2), valor_total numeric(38,2), cliente_id bigint not null, endereco_cidade_id bigint, forma_pagamento_id bigint not null, restaurante_id bigint not null, primary key (id));
create table permissao (id bigserial not null, descricao varchar(200), nome varchar(50) not null, primary key (id));
create table produto (id bigserial not null, ativo boolean not null, descricao varchar(255), nome varchar(50) not null, preco numeric(38,2) not null, restaurante_id bigint not null, primary key (id));
...
alter table if exists foto_produto add constraint FK4npv85rdbnvn7a6uc4o7jujy1 foreign key (produto_id) references produto;
...
alter table if exists pedido add constraint FK37ms39e5dvx6m05hftvx9uavk foreign key (cliente_id) references usuario;
